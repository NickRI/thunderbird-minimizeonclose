...
xpi: dist/minimizeonclose.xpi
xpi-unsupported: dist/minimizeonclose-unsupported.xpi
...
dist/minimizeonclose-unsupported.xpi: $(SRCFILES) build/unsupported/manifest.json LICENSE
	mkdir -p "$(@D)"
	rm -f "$@"
	cd src ; zip -9X "../$@" $(SRCFILES:src/%=%)
